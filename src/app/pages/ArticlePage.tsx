...
const categoryColor: Record<string, string> = {
  โปรโมชั่น: "bg-orange-100 text-orange-700",
  ข่าวสาร: "bg-green-100 text-green-700",
  บทความ: "bg-blue-100 text-[#0B5ED7]",
};
...
function ShareButtons({
  ...
  const btn =
    "inline-flex items-center justify-center w-10 h-10 rounded-full border border-gray-200 bg-white text-gray-600 hover:text-[#0B5ED7] hover:border-[#0B5ED7] transition-colors";
...
export function ArticlePage() {
    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-20 text-center">
          ...
          <Link
            to="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#0B5ED7] text-white hover:bg-[#094fb8] transition-colors"
          >
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
        ...
        {loading || !article ? (
          ...
          <>
            ...
            <nav className="flex items-center gap-1.5 text-sm text-gray-500 mb-6 flex-wrap">
              <Link to="/" className="hover:text-[#0B5ED7]">
                ข่าวสาร
              </Link>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-10 pt-6 border-t border-gray-100">
                    ...
                    {article.tags.map((t) => (
                      <Link
                        ...
                        className="text-xs px-3 py-1 rounded-full bg-gray-100 text-gray-700 hover:bg-[#0B5ED7] hover:text-white transition-colors"
                      >
                        {t}
                      </Link>
                    ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                    {prev ? (
                      <Link
                        ...
                        className="group rounded-xl border border-gray-200 p-4 hover:border-[#0B5ED7] hover:shadow-sm transition-all"
                      >
                        <div className="text-xs text-gray-500 inline-flex items-center gap-1 mb-1">
                          <ChevronLeft className="w-3 h-3" /> บทความก่อนหน้า
                        </div>
                        <div className="text-sm text-gray-900 group-hover:text-[#0B5ED7] line-clamp-2">
                          {prev.title}
                        </div>
                      </Link>
                    ) : (
                    ...
                    {next ? (
                      <Link
                        ...
                        className="group rounded-xl border border-gray-200 p-4 hover:border-[#0B5ED7] hover:shadow-sm transition-all md:text-right"
                      >
                    ...
                    <ul className="space-y-3">
                      {related.map((r) => (
                        <li key={r.id}>
                          <Link
                            to={`/article?id=${r.id}`}
                            className="flex gap-3 p-2 rounded-lg hover:bg-[#F5F7FA] transition-colors group"
                          >
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                              <h3 className="text-sm text-gray-900 line-clamp-2 group-hover:text-[#0B5ED7]">
                                {r.title}
                              </h3>
      ...
      <style>{`
        .article-body h2 { font-size: 1.5rem; font-weight: 700; color: #111827; margin-top: 2rem; margin-bottom: 0.75rem; line-height: 1.3; scroll-margin-top: 96px; }
        .article-body h3 { font-size: 1.2rem; font-weight: 600; color: #111827; margin-top: 1.5rem; margin-bottom: 0.5rem; scroll-margin-top: 96px; }
        .article-body p { margin-bottom: 1rem; }
        .article-body ul, .article-body ol { margin: 1rem 0 1rem 1.5rem; padding-left: 0.5rem; }
        .article-body ul { list-style: disc; }
        .article-body ol { list-style: decimal; }
        .article-body li { margin-bottom: 0.4rem; }
        .article-body blockquote { border-left: 3px solid #0B5ED7; background: #F5F7FA; padding: 0.75rem 1rem; margin: 1.25rem 0; color: #374151; border-radius: 0 8px 8px 0; }
        .article-body a { color: #0B5ED7; text-decoration: underline; text-underline-offset: 2px; }
        .article-body img { max-width: 100%; height: auto; border-radius: 8px; margin: 1rem 0; }
        .article-body code { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; background: #F5F7FA; padding: 0.1rem 0.4rem; border-radius: 4px; font-size: 0.9em; }
        ...
        .article-body th { background: #F5F7FA; }
      `}</style>
    </div>
  );
}