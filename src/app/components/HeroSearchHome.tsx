    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        <select
          ...
          className="w-full appearance-none px-3 py-2.5 rounded-xl border border-gray-200 bg-white hover:border-[#550000] text-sm text-gray-700 focus:outline-none transition-colors cursor-pointer pr-8"
        >
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="relative py-2">
        <div className="h-1.5 bg-gray-100 rounded-full">
          <div className="h-1.5 bg-[#550000] rounded-full" style={{ width: `${pct}%` }} />
        </div>
        <input
          type="range"
          min={min}
        ...
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-white border-2 border-[#550000] shadow-sm pointer-events-none"
...
function PriceRangeSlider({ minVal, maxVal, onMinChange, onMaxChange }: { minVal: number; maxVal: number; onMinChange: (v: number) => void; onMaxChange: (v: number) => void }) {
  ...
  const thumbCls =
    ...
    "[&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-[#550000] " +
    ...
    "[&::-moz-range-thumb]:bg-white [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-[#550000] " +
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-semibold text-[#550000]">
      ...
      <div className="relative h-8 flex items-center px-2.5">
        ...
        <div
          className="absolute h-1.5 bg-[#550000] rounded-full"
...
const privileges = [
  ...
  { Icon: ShieldCheck, label: "ความปลอดภัย",  color: "#550000", bg: "#E0F2FE" },
      ...
      <div className="relative mx-auto px-4 md:px-6">
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_16px_-4px_rgba(0,0,0,0.20)] border border-gray-100 p-4 md:p-8">
          ...
          <div className="mt-5 md:mt-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="text-xs text-[#550000] hover:text-[#3D0000] font-medium inline-flex items-center gap-1 transition-colors"
              >
          ...
          <div className={`mt-4 grid grid-cols-1 sm:grid-cols-3 gap-5 p-4 md:p-5 bg-[#F9FAFC] rounded-xl border border-gray-100 transition-all ${advanced ? "block" : "hidden"}`}>
            <RangeSlider label="Download ขั้นต่ำ (Mbps)" min={0} max={2000} unit="Mbps" defaultValue={0} onChange={setMinDownload} accentColor="#550000" />
            <RangeSlider label="Upload ขั้นต่ำ (Mbps)" min={0} max={1000} unit="Mbps" defaultValue={0} onChange={setMinUpload} accentColor="#550000" />
            <RangeSlider label="สัญญาไม่เกิน (เดือน)" min={0} max={24} unit="เดือน" defaultValue={24} onChange={setMaxContract} accentColor="#550000" />
          </div>
